...
func (module *PP2PLink) Init(address string) {
	module.InitD(address, true) //           //default - inicia com debug on
}
func (module *PP2PLink) InitD(address string, _dbg bool) { // permite especificar se deve gerar mensagens de debug ou nao
	if module.Run { //        // protegendo contra init duplicado.   nao deveria ocorrer
		return
	}
	module.Cache = make(map[string]net.Conn) // inicia cache de conexoes abertas
	module.Run = true
	module.dbg = _dbg //                     // deve print mensagens ou nao
	module.outDbg(" Init PP2PLink!")
